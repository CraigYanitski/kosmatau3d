...
x_length = 1
y_length = 5 # 55 # edge length of map (projection)
z_length = 5 # 80 
x_offset = 0
y_offset = 0# 32 
z_offset = 0 #20
...
theta = 2. # 15 # angle between los and edge-on part of the pdr. in degree
# IF at y=0
...
_globals['compound']['losoffsets'] = np.array([[0, -3]])
...
_globals['plots']['plotrange'] = np.array([[-30, 30], [-35, 25], [-20, 60]])
...
_globals['sigma']['sigma_inter_j'] = np.array([1.67/2.3548], float)
...
_globals['sigma']['sigma_ens_inter_j'] = np.array([3.635/2.3548], float)
...
for i in np.arange(_globals['compound']['npoints']):
    ens.append(Ensemble(i, \
               rho_ens_clumps  = 1. * 10**6, \
               rho_ens_inter   = 2. * 10**4, \
...
for i in np.arange(_globals['compound']['npoints']):  
    ...
    z = coords[i][2]
    if z >= 0 and y <= 0: 
        if y > -np.tan(theta * np.pi/180.) * z - (d_clumps-1):  
            f = 0.0
        else: f = 1. 
    elif z < 0 and y > 0: 
        if z >= -(d_clumps-1): f = 0.0
        else: f = 1
    elif z <= 0 and y <= 0: 
        if (z >= -(d_clumps-1) and y > -(d_clumps-1)):  
            f = 0.0
        else: f = 1. 
    ens[i].Mens_clumps = 0.00173 * 2.5 * f
...
fig2 = pf.plot_geo(_globals['compound']['coordinates'], \
           ...
           title = 'mass distribution', cbarLabel = "mass")
p.show()
