...
z_disk = 0.75 # thickness /  scaleheight of disk
...
for x in np.arange(x_length) - x_offset:
  ...
  for y in np.arange(y_length) - y_offset:
        for z in np.arange(z_length) - z_offset:
            ...
            if calc_r (x,y,z,inc_disk) < r_disk and abs(calc_h(x,y,z,inc_disk)) < z_disk and z < 8.5:
                coords.append([x, y, z])
...
#print "number\n", number
print 'coords', coords
_globals['compound']['coordinates'] = np.array(coords) #list/tuples to matrix
print 'array', _globals['compound']['coordinates']
...
for i in np.arange(len(dens_list_r)):
  dens_profile[i,0] = float(dens_list_r[i])
...
for i in np.arange(len(mass_list_r)):
  mass_profile_inter[i,0] = float(mass_list_r[i])
...
if write_fits == 1:
  ...
  for i in np.arange(_globals['compound']['npoints']): # fill grids with values
    ...
    #print "z", z - z_offset
    mass_data[z][y][x] = ens[i].Mens_clumps + ens[i].Mens_inter # write mass data to array coordiantes
    mass_clump_data[z][y][x] = ens[i].Mens_clumps
    mass_inter_data[z][y][x] = ens[i].Mens_inter
    
    vz_data[z][y][x] = ens[i].velocity[2] # velocitydata in z
    density_data[z][y][x] = ens[i].rho_ens_clumps # densitydata
    FUV_data[z][y][x] = ens[i].FUV #FUV data
    disp_data[z][y][x] = ens[i].v_dispersion
