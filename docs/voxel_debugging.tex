...
\begin{document}
    ...
    \[
    \epsilon_{\nu,j,i} (v_{obs}) = \frac{k_{j,i} I_{\nu,j} \tau_{\nu,j}}{\left[ 1 - \exp(- \tau_{\nu,j}) \right]} \frac{2 \pi R_{cl,j}^2}{V_{j}} \mathrm{exp} \left( \frac{(v_i-v_{obs})^2}{2\sigma_{cl,j}^2} \right).
    \]
    ...
    \[
    \left< \kappa_\nu \right>_{vox} (v_{obs}) = -\mathrm{ln} \Bigg\{ \sum_i \bigg[ \Big( \prod_j p_{j,i}(k_{j,i}) \Big) \ \mathrm{exp} \Big( - \sum_j \tau_{\nu,j,i} (v_{obs}) \times \frac{2 \pi R_{cl,j}^2}{V_{voxel}} \times \Delta s \Big) \bigg] \Bigg\}.
    \]
