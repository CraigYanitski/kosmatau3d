...
def fuvAbsorption():
  ...
  pf.progressBar(0, len(number_ensembles), length=100)
  ...
  ##
        ...
        #print 'number', number
        AFUVCl = 10**(griddata(RhoMassAFUV[:,[0,1]], \
        ...
        for c in range(len(combis)):
          ...
          for inter in range(numberInt):
            AFUVtot = AFUVtot + AFUVCl[inter] * combis[c][inter]
            # print 'ptot', ptot
            ptot = ptot * float(probabilityTab[inter]\
                                (int(combis[c][inter])))
        ...
        # pause = input()
        if gbl._globals['verbose']: print('to obtain exact results p needs to be close to 1. Otherwise increase nsigma')
    ...
    pf.progressBar(i+1, len(number_ensembles), length=100)
