...
higal_wavelengths = {"70um": "70.79um", "160um": "158.5um", "250um": "240um"}
...
def determine_rms(hdul, mission="", file=""):
    ...
    if mission == "GOT C+":
        ...
        i_mask = (
            np.where((vel >= -150) & (vel <= 50))[0].reshape(-1, 1, 1),
            np.where((lat <= 90) & (lat >= -90))[0].reshape(1, -1, 1),
            np.where((lon <= -10))[0].reshape(1, 1, -1),
        )
        rms_mask[i_mask] = 1
        i_mask = (
            np.where((vel >= -100) & (vel <= 150))[0].reshape(-1, 1, 1),
            np.where((lat <= 90) & (lat >= -90))[0].reshape(1, -1, 1),
            np.where((lon >= -10) & (lon <= 10))[0].reshape(1, 1, -1),
        )
        rms_mask[i_mask] = 1
        i_mask = (
            np.where((vel >= -50) & (vel <= 200))[0].reshape(-1, 1, 1),
            np.where((lat <= 90) & (lat >= -90))[0].reshape(1, -1, 1),
            np.where((lon >= 10))[0].reshape(1, 1, -1),
        )
        rms_mask[i_mask] = 1
        ...
        i_mask = (
            np.where((vel_co >= -50) & (vel_co <= 150))[0].reshape(-1, 1, 1),
            np.where((lat_co <= 90) & (lat_co >= -90))[0].reshape(1, -1, 1),
            np.where((lon_co >= -180) & (lon_co < -70))[0].reshape(1, 1, -1),
        )
        rms_mask[i_mask] = 1
        i_mask = (
            np.where((vel_co >= -220) & (vel_co <= 125))[0].reshape(-1, 1, 1),
            np.where((lat_co <= 90) & (lat_co >= -90))[0].reshape(1, -1, 1),
            np.where((lon_co >= -70) & (lon_co < -7))[0].reshape(1, 1, -1),
        )
        rms_mask[i_mask] = 1
        i_mask = (
            np.where((vel_co >= -310) & (vel_co <= 320))[0].reshape(-1, 1, 1),
            np.where((lat_co <= 90) & (lat_co >= -90))[0].reshape(1, -1, 1),
            np.where((lon_co >= -7) & (lon_co < 7))[0].reshape(1, 1, -1),
        )
        rms_mask[i_mask] = 1
        i_mask = (
            np.where((vel_co >= -80) & (vel_co <= 200))[0].reshape(-1, 1, 1),
            np.where((lat_co <= 90) & (lat_co >= -90))[0].reshape(1, -1, 1),
            np.where((lon_co >= 7) & (lon_co < 50))[0].reshape(1, 1, -1),
        )
        rms_mask[i_mask] = 1
        i_mask = (
            np.where((vel_co >= -60) & (vel_co <= 75))[0].reshape(-1, 1, 1),
            np.where((lat_co <= 90) & (lat_co >= -90))[0].reshape(1, -1, 1),
            np.where((lon_co >= 50) & (lon_co < 70))[0].reshape(1, 1, -1),
        )
        rms_mask[i_mask] = 1
        i_mask = (
            np.where((vel_co >= -90) & (vel_co <= 25))[0].reshape(-1, 1, 1),
            np.where((lat_co <= 90) & (lat_co >= -90))[0].reshape(1, -1, 1),
            np.where((lon_co >= 70) & (lon_co <= 180))[0].reshape(1, 1, -1),
        )
        rms_mask[i_mask] = 1
...
def regrid_observations(
    path="/media/hpc_backup/yanitski/projects/" + "pdr/observational_data/MilkyWay/",
    mission=None,
    nu_planck=713e9,
    target_header=None,
    target_kernel=None,
    output_file="obs_regridded.fits",
    cal_error=0,
    sig_th=3,
):
    """
    ...
    if mission == None or mission[0] == "":
        ...
        print(
            "Invalid mission... {} observations do not exist or "
    ...
    if target_header["NAXIS"] == 3:
        target_vel = np.linspace(
            target_header["CRVAL3"]
            - target_header["CDELT3"] * (target_header["CRPIX3"] - 1),
            target_header["CRVAL3"]
            + target_header["CDELT3"]
            * (target_header["NAXIS3"] - target_header["CRPIX3"]),
            num=target_header["NAXIS3"],
        )
    else:
        print(
            "Error: Please enter a target velocity to regrid the "
    ...
    for survey in mission:
        ...
        for file in files:
            ...
            # Grid data and RMS
                ...
                lat = np.linspace(
                    obs[0].header["CRVAL3"]
                    - obs[0].header["CDELT3"] * (obs[0].header["CRPIX3"] - 1),
                    obs[0].header["CRVAL3"]
                    + obs[0].header["CDELT3"]
                ...
                obs_data = spectres(
                    target_vel,
                    vel,
                    np.nan_to_num(obs[0].data, nan=0),
                    fill=0,
                    verbose=False,
                )
                obs_data = np.nan_to_num(
                    obs_data.reshape(-1, obs_data.shape[-1]), nan=0
                )
                ...
                co1_gridder_err.grid(
                    lon_mesh.flatten(), lat_mesh.flatten(), obs_error[:, 0]
                )
                ...
                # Open file
                obs = fits.open(path + survey + "/" + file)
                obs_error = np.nanmean(
                    fits.open(path + survey + "/" + file.replace("_Vfull", ".sigma"))[
                        0
                    ].data
                )
                ...
                obs_data = spectres(
                    target_vel,
                    vel,
                    np.nan_to_num(obs[0].data.T, nan=0),
                    fill=0,
                    verbose=False,
                )
                obs_data = obs_data.reshape(-1, obs_data.shape[-1])
                obs_error = fits.open(
                    path + survey + "/" + file.replace("_Vfull", ".sigma")
                )[0].data.reshape(-1)
                ...
                if "12" in transition:
                    co1_gridder.grid(lon_mesh.flatten(), lat_mesh.flatten(), obs_data)
                    co1_gridder_err.grid(
                        lon_mesh.flatten(), lat_mesh.flatten(), obs_error
                    )
                elif "13" in transition:
                    ico1_gridder.grid(lon_mesh.flatten(), lat_mesh.flatten(), obs_data)
                    ico1_gridder_err.grid(
                ...
                obs_data = obs_data.reshape(-1, obs_data.shape[-1])
                obs_rms_error = determine_rms(obs, mission=survey, file=file)[
                    0
                ].data.reshape(-1, 1)
                if cal_error:
                    ...
                    obs_error[idx_sig] = (
                        cal_error * obs_data[idx_sig] + obs_error[idx_sig]
                    )
                else:
                    obs_error = obs_rms_error
                i_nan = np.isnan(obs_error)
                ...
                lat_mesh, lon_mesh = np.degrees(
                    hp.pix2ang(nside=nside, ipix=np.arange(npix), nest=nest)
                )
                ...
                grid_hdu = fits.PrimaryHDU(
                    data=dust_gridder.get_datacube(), header=fits.Header(temp_header)
                )
                ...
                grid_hdu.writeto(
                    path
                    + survey
                    + "/regridded/temp/planck_"
                    + file.split("_")[2]
                    + "_regridded.fits",
                    overwrite=True,
                    output_verify="ignore",
                )
                grid_hdu_err.writeto(
                    path
                    + survey
                    + "/regridded/temp/planck_"
                    + file.split("_")[2]
                    + "_regridded_error.fits",
                    overwrite=True,
                    output_verify="ignore",
                )
                del obs_data
                ...
                linfrq = np.array(
                    [
                        obs[0].header[key]
                        for key in obs[0].header.keys()
                        if "LINFRQ" in key
                    ]
                )
                obs_data = (
                    np.nan_to_num(obs[1].data["LINE_FLU"], nan=0)
                    * (2.9979**3)
                    / (linfrq**3)
                    / 2
                    / 1.38
                ...
                obs_error = (
                    np.nan_to_num(obs[1].data["LINE_FL2"], nan=0)
                    * (2.9979**3)
                    / (linfrq**3)
                    / 2
                    / 1.38
                ...
                temp_header["TRANSI"] = ", ".join(
                    "{}".format(_) for _ in np.arange(len(transitions))
                )
                grid_hdu = fits.PrimaryHDU(
                    data=gridder.get_datacube(), header=fits.Header(temp_header)
                )
                grid_hdu_err = fits.PrimaryHDU(
                    data=gridder_err.get_datacube(), header=fits.Header(temp_header)
                )
                grid_hdu.writeto(
                    path
                    + survey
                ...
                obs = fits.open(path + survey + "/" + file)
                pixcoord = fits.open(
                    path + survey + "/additional_files/DIRBE_SKYMAP_INFO.FITS"
                )
                linfrq = 2.9979e5 / 240
                # convert the relevant DIRBE band to GHz
                obs_data = (
                    np.nan_to_num([obs[1].data["Resid"]], nan=0).T
                    * (2.9979**2)
                    / (linfrq**2)
                    / 2
                    / 1.38
                ...
                obs_error = (
                    np.nan_to_num([obs[1].data["StdDev"]], nan=0).T
                    * (2.9979**2)
                    / (linfrq**2)
                    / 2
                    / 1.38
        ...
        if CO1:
            ...
            temp_header["TRANSI"] = "0"
            grid_hdu = fits.PrimaryHDU(
                data=co1_gridder.get_datacube(), header=fits.Header(temp_header)
            )
            grid_hdu_err = fits.PrimaryHDU(
                data=co1_gridder_err.get_datacube(), header=fits.Header(twod_header)
            )
            grid_hdu.writeto(
                path + survey + "/regridded/temp/" + "co1_test_regridded.fits",
                overwrite=True,
                output_verify="ignore",
            )
            grid_hdu_err.writeto(
                path + survey + "/regridded/temp/" + "co1_test_regridded_error.fits",
                overwrite=True,
                output_verify="ignore",
            )
        if CO2:
            ...
            temp_header["TRANSI"] = "0"
            grid_hdu = fits.PrimaryHDU(
                data=co2_gridder.get_datacube(), header=fits.Header(temp_header)
            )
            grid_hdu_err = fits.PrimaryHDU(
                data=co2_gridder_err.get_datacube(), header=fits.Header(twod_header)
            )
            grid_hdu.writeto(
                path + survey + "/regridded/temp/" + "co2_test_regridded.fits",
                overwrite=True,
                output_verify="ignore",
            )
            grid_hdu_err.writeto(
                path + survey + "/regridded/temp/" + "co2_test_regridded_error.fits",
                overwrite=True,
                output_verify="ignore",
            )
        if iCO1:
            ...
            temp_header["TRANSI"] = "0"
            grid_hdu = fits.PrimaryHDU(
                data=ico1_gridder.get_datacube(), header=fits.Header(temp_header)
            )
            grid_hdu_err = fits.PrimaryHDU(
                data=ico1_gridder_err.get_datacube(), header=fits.Header(twod_header)
            )
            grid_hdu.writeto(
                path + survey + "/regridded/temp/" + "13co1_test_regridded.fits",
                overwrite=True,
                output_verify="ignore",
            )
            grid_hdu_err.writeto(
                path + survey + "/regridded/temp/" + "13co1_test_regridded_error.fits",
                overwrite=True,
                output_verify="ignore",
            )
        if iCO2:
            ...
            temp_header["TRANSI"] = "0"
            grid_hdu = fits.PrimaryHDU(
                data=ico2_gridder.get_datacube(), header=fits.Header(temp_header)
            )
            grid_hdu_err = fits.PrimaryHDU(
                data=ico2_gridder_err.get_datacube(), header=fits.Header(twod_header)
            )
            grid_hdu.writeto(
                path + survey + "/regridded/temp/" + "13co2_test_regridded.fits",
                overwrite=True,
                output_verify="ignore",
            )
            if cal_error:
                ...
                grid_hdu_err.writeto(
                    ...
                    + survey
                    + "/regridded/temp/"
                    + "13co2_test_regridded_error.fits",
                    overwrite=True,
                    output_verify="ignore",
                )
        if hi:
            ...
            temp_header["TRANSI"] = "0"
            grid_hdu = fits.PrimaryHDU(
                data=hi_gridder.get_datacube(), header=fits.Header(temp_header)
            )
            grid_hdu_err = fits.PrimaryHDU(
                data=hi_gridder_err.get_datacube(), header=fits.Header(twod_header)
            )
            grid_hdu.writeto(
                path + survey + "/regridded/temp/" + "hi_test_regridded.fits",
                overwrite=True,
                output_verify="ignore",
            )
            grid_hdu_err.writeto(
                path + survey + "/regridded/temp/" + "hi_test_regridded_error.fits",
                overwrite=True,
                output_verify="ignore",
            )
        # if dust:
...
def combine_regridded(
    ...
    if "/COBE/" in path or "/COBE-FIRAS/" in path:
        ...
        output_data = np.zeros(
            (target_header["NAXIS3"], target_header["NAXIS2"], target_header["NAXIS1"])
        )

    lon = np.linspace(
        target_header["CRVAL1"]
        - target_header["CDELT1"] * (target_header["CRPIX1"] - 1),
        target_header["CRVAL1"]
        + target_header["CDELT1"] * (target_header["NAXIS1"] - target_header["CRPIX1"]),
        num=target_header["NAXIS1"],
    )
    lat = np.linspace(
        target_header["CRVAL2"]
        - target_header["CDELT2"] * (target_header["CRPIX2"] - 1),
        target_header["CRVAL2"]
        + target_header["CDELT2"] * (target_header["NAXIS2"] - target_header["CRPIX2"]),
    ...
    for file in files:
        ...
        if "/COBE/" in path or "/COBE-FIRAS/" in path:
            ...
            gridder.grid(
                lon_mesh.flatten(),
                lat_mesh.flatten(),
                np.nan_to_num(obs[0].data.reshape(-1, obs[0].shape[-1])),
            )
            # i = output_data == 0
...
def view_observation(
    ...
    filename="CO1_obs_regridded.fits",
    plot="integrated",
    integrate_b=[],
    i_lat=None,
    ...
    lon = np.linspace(
        header["CRVAL1"] - header["CDELT1"] * (header["CRPIX1"] - 1),
        header["CRVAL1"] + header["CDELT1"] * (header["NAXIS1"] - header["CRPIX1"]),
        num=header["NAXIS1"],
    )
    lat = np.linspace(
        header["CRVAL2"] - header["CDELT2"] * (header["CRPIX2"] - 1),
    ...
    if (
        (mission != "COBE-FIRAS")
        and (mission != "COBE-DIRBE")
        and (mission != "Planck")
        and not ("error" in filename)
    ):
        print("spectroscopic data")
        if transition != header["TRANSL"]:
            print(
                "Line transition not in specified file. Please supply the correct line/file."
            )
        ...
        del twod_header["CDELT3"]
        vel = np.linspace(
            header["CRVAL3"] - header["CDELT3"] * (header["CRPIX3"] - 1),
        ...
        if not np.any(i_line):
            print(
                "Line transition {} not in specified file. Please supply the correct line/file.".format(
                    transition
                )
            )
    ...
    if plot == "integrated":
        ...
        if logval:
            cm = ax.imshow(
                np.log10(cygrid_integrated_data), vmin=vmin, vmax=vmax, cmap=cmap
            )
        else:
        ...
        if clabel:
            ...
            cb.ax.set_ylabel(
                r"Integrated Intensity ($K \frac{km}{s}$)", fontsize=fontsize
            )
    elif plot == "pv" or plot == "PV":
        print(
            "Number of finite-valued sightlines:",
            cygrid_data.size - np.where(np.isnan(cygrid_data))[0].size,
        )
        if type(i_lat) != int:
        ...
        if integrate_b:
            ...
            if "error" in filename:
                print(
                    ...
                    np.nanmin(
                        np.log10(
                            np.nansum(
                                cygrid_data[(integrate_b[0] - 1) : integrate_b[1], :],
                                axis=0,
                            )
                        )
                    ),
                ...
                cm = ax.pcolormesh(
                    ...
                    [1],
                    np.nansum(
                        [cygrid_data[(integrate_b[0] - 1) : integrate_b[1], :]], axis=1
                    ),
                    vmin=vmin,
                    vmax=vmax,
                    shading="auto",
            ...
            if "error" in filename:
                print(
                    np.nanmin(cygrid_data[int(lat.size / 2), :]),
        ...
        if ylabel:
            ...
            ax.set_ylabel(
                r"$T_{int} \ \left( K \frac{km}{s} \right)$", fontsize=fontsize
            )
    else:
            ...
            for i in range(int(np.ceil(data_copy.shape[0] / 2))):
                for j in range(int(np.ceil(data_copy.shape[1] / 2))):
                    for k in range(int(data_copy.shape[2])):
                        ...
                        idx = (
                            np.array([i, i, -1 - i, -1 - i]),
                            np.array([j, -1 - j, j, -1 - j]),
                            np.array([k, k, -1 - k, -1 - k]),
                        )
                        if np.isnan(data_copy[idx]).all():
                            continue
                        avg = np.nanmean(data_copy[idx])
...
def model_selection_old(
    path="/mnt/hpc_backup/yanitski/projects/pdr/KT3_history/MilkyWay",
    missions=None,
    lat=None,
    model_dir="",
    model_param=[[]],
    ...
    if missions == "" or missions == None or missions == []:
        ...
        if "COBE" in missions:
            if "COBE-FIRAS" in missions:
                missions.remove("COBE-FIRAS")
            if "COBE-DIRBE" in missions:
                missions.remove("COBE-DIRBE")
    ...
    model_params = np.transpose(
        [model_params[n].flatten() for n in range(len(model_param))]
    )
    ...
    for survey in missions:
        ...
        if path[-1] != "/":
            path += "/"
        directory = (
            path.replace("KT3_history", "observational_data")
            + survey
            + "/regridded/temp/"
        )
        # if mission == 'COBE-FIRAS':
        ...
        for file in files:
            ...
            if ".fits" in file:
                ...
                obs_error = fits.open(directory + file.replace(".fits", "_error.fits"))[
                    0
                ].data
            ...
            if survey == "COBE-DIRBE" or survey == "COBE-FIRAS":
                if file == "craig.idl":
                    ...
                    obs_data = (
                        obs["amplitude"] / (2.9979**3) * (linfrq**3) * 2 * 1.38 / 10**8
                    )
                    obs_error = (
                        obs["sigma"] / (2.9979**3) * (linfrq**3) * 2 * 1.38 / 10**8
                    )
                    lon_obs = obs["long"]
                    lat_obs = np.array([0])
            ...
            for i, transition in enumerate(transitions):
                ...
                if (survey == "COBE-DIRBE") or (survey == "COBE-FIRAS"):
                    if ".idl" in file:
                        obs_error_conf = error_correction(
                            obs_data[:, int(transition_indeces[i])], conf="axisymmetric"
                        )
                        vmin = obs_data[:, int(transition_indeces[i])].min()
                        vmax = obs_data[:, int(transition_indeces[i])].max()
                    else:
                        obs_error_conf = error_correction(
                            obs_data[int(transition_indeces[i]), :, :],
                            conf="axisymmetric",
                        )
                        vmin = obs_data[int(transition_indeces[i]), :, :].min()
                        vmax = obs_data[int(transition_indeces[i]), :, :].max()
                ...
                for param in model_params:
                    ...
                    if survey in ("Planck", "COBE-DIRBE"):
                        ...
                        i_spec = np.where(
                            np.asarray(model[1].header["SPECIES"].split(", "))
                            == transition
                        )[0]
                    if i_spec.size == 0:
                    ...
                    lat_model = (
                        np.linspace(
                            ...
                            - model[1].header["CDELT3"]
                            * (model[1].header["CRPIX3"] - 0.5),
                            model[1].header["CRVAL3"]
                            + model[1].header["CDELT3"]
                            * (
                                model[1].header["NAXIS3"]
                                - model[1].header["CRPIX3"]
                                - 0.5
                            ),
                    ...
                    lon_model = (
                        np.linspace(
                            ...
                            - model[1].header["CDELT2"]
                            * (model[1].header["CRPIX2"] - 0.5),
                            model[1].header["CRVAL2"]
                            + model[1].header["CDELT2"]
                            * (
                                model[1].header["NAXIS2"]
                                - model[1].header["CRPIX2"]
                                - 0.5
                            ),
                    ...
                    if (
                        ...
                        i_lon_model = np.linspace(
                            0, lon_model.size - 1, num=lon_obs.size, dtype=int
                        )
                        i_lat_model = (lat_model >= lat_min) & (lat_model <= lat_max)
                        i_lat_obs = (lat_obs >= lat_model[i_lat_model].min()) & (
                            lat_obs <= lat_model[i_lat_model].max()
                        )
                    else:
                        i_lat_model = np.where(
                            (lat_model >= lat_min) & (lat_model <= lat_max)
                        )[0]
                        i_lat_obs = np.where(
                            (lat_obs >= lat_model[i_lat_model].min())
                            & (lat_obs <= lat_model[i_lat_model].max())
                        )[0]
                    ...
                    if (survey == "COBE-DIRBE") or (survey == "COBE-FIRAS"):
                        idx_t = np.ix_(
                            np.arange(vel_model.size), i_lat_model, i_lon_model, i_spec
                        )
                        ...
                        #       i_lat_obs.size)
                        model_data = model[1].data[idx_t] - model[2].data[idx_d]
                        model_interp = trapz(
                            model_data,
                            vel_model,
                            axis=model_data.shape.index(vel_model.size),
                        )
                        if ".idl" in file:
                            idx_obs = np.ix_(
                                np.arange(obs_data.shape[0]),
                                [int(transition_indeces[i])],
                            )
                            ...
                            idx_obs = np.ix_(
                                [int(transition_indeces[i])],
                                i_lat_obs,
                                np.arange(obs_data.shape[2]),
                            )
                            obs_data_final = obs_data[idx_obs][
                                0, :, :
                            ]  # [int(transition_indeces[i]), i_lat_obs, :]
                            obs_error_final = np.sqrt(
                                obs_error**2 + obs_error_conf**2
                            )[idx_obs][
                                0, :, :
                            ]  # [int(transition_indeces[i]), i_lat_obs, :]
                        ...
                        model_interp = model[2].data[idx_d][
                            :, :, 0
                        ]  # [0, i_lat_model, :, i_spec]
                        idx_obs = np.ix_(i_lat_obs, np.arange(obs_data.shape[1]))
                        obs_data_final = obs_data[idx_obs]  # [i_lat_obs, :]
                        obs_error_final = np.sqrt(obs_error**2 + obs_error_conf**2)[
                            idx_obs
                        ]  # [i_lat_obs, :]
                        # print(model_interp.shape, obs_data_final.shape)
                        ...
                        # input()
                        idx_t = np.ix_(
                            np.arange(vel_model.size),
                            i_lat_model,
                            np.arange(lon_model.size),
                            i_spec,
                        )
                        ...
                        model_data = model[1].data[idx_t] - model[2].data[idx_d]
                        idx_obs = np.ix_(
                            np.arange(vel_obs.size), i_lat_obs, np.arange(lon_obs.size)
                        )
                        idx_map = np.ix_(i_lat_obs, np.arange(lon_obs.size))
                        obs_data_temp = obs_data[
                            idx_obs
                        ]  # np.swapaxes(obs_data[idx_obs], 0, 1)
                        if comp_type == "integrated":
                            ...
                            model_interp = np.trapz(
                                model_data,
                                vel_model,
                                axis=model_data.shape.index(vel_model.size),
                            )
                            obs_data_final = np.trapz(
                                obs_data_temp,
                                vel_obs,
                                axis=obs_data_temp.shape.index(vel_obs.size),
                            )
                            obs_error_final = (
                                len(vel_obs)
                                * np.sqrt(obs_error**2 + obs_error_conf**2)[idx_obs]
                            )
                        elif comp_type == "pv":
                            model_interp = model_data[
                                :, :, :, 0
                            ]  # np.swapaxes(model_data, 0, 1)
                            ...
                            print(
                                "ERROR >> Comparison type {} not valid; ".format(
                                    comp_type
                                )
                        ...
                        if debug:
                            ...
                            print(
                                vel.shape, lon.shape, obs_data.shape, model_interp.shape
                            )
                            ...
                            cb = plt.pcolormesh(
                                ...
                                norm=colors.SymLogNorm(
                                    base=10, linthresh=0.1, vmin=0, vmax=vmax
                                ),
                                shading="auto",
                                cmap="viridis",
                            )
                            ...
                            cb = plt.pcolormesh(
                                ...
                                obs_data,
                                norm=colors.SymLogNorm(
                                    base=10, linthresh=0.1, vmin=vmin, vmax=vmax
                                ),
                                shading="auto",
                                cmap="viridis",
                                alpha=0.25,
                    ...
                    chi2[i_signal] = (
                        obs_data_final[i_signal] - model_interp[i_signal]
                    ) ** 2 / obs_error_final[i_signal] ** 2
                    loglikelihood[i_signal] = -chi2[i_signal] / 2 - 0.5 * np.log(
                        2 * np.pi * obs_error_final[i_signal] ** 2
                    )
                    # elif survey == 'Planck':
                    ...
                    np.save(
                        path
                        + "fit_results/{}/{}/{}/{}_chi2.npy".format(
                            survey,
                            file.replace(".fits", ""),
                            transition,
                            dir_model.replace("/", "").replace(
                    ...
                    np.save(
                        path
                        + "fit_results/{}/{}/{}/{}_loglikelihood.npy".format(
                            survey,
                            file.replace(".fits", ""),
                            transition,
                            dir_model.replace("/", "").replace(
                    ...
                    if PLOT:
                        if (survey == "COBE-DIRBE") or (survey == "COBE-FIRAS"):
                            ...
                            cm2 = ax2.scatter(
                                lon_obs,
                                lat_obs,
                                c=obs_data[:, :, int(transition_indeces[i])],
                                norm=colors.SymLogNorm(
                                    base=10, linthresh=0.1, vmin=0, vmax=vmax
                                ),
                                cmap=cmap,
                            ...
                            ax.set_xlabel(r"Longitude ($^\circ$)", fontsize=16)
                            ax.set_ylabel(
                                r"Radial Velocity ($\frac{km}{s}$)", fontsize=16
                            )
                            ax2.set_xlabel(r"Longitude ($^\circ$)", fontsize=16)
                            ax2.set_ylabel(
                                r"Radial Velocity ($\frac{km}{s}$)", fontsize=16
                        ...
                        fig.savefig(
                            path
                            + "fit_results/{}/{}/{}/synthetic_observation_{}.png".format(
                                survey,
                                file.replace(".fits", ""),
                                transition,
                                dir_model.split("/")[0],
                        ...
                        fig2.savefig(
                            path
                            + "fit_results/{}/{}/{}/synthetic_observation_smoothed.png".format(
                ...
                i_bestfit2 = np.where(loglikelihood_grid == np.max(loglikelihood_grid))[
                    0
                ][0]
                # print(i_bestfit2)
                print(
                    "\n    The best-fitting model for {} with transition {}\n".format(
                        file.replace(".fits", ""), transition
                    )
                    + "  has parameters "
                    + model_dir.format(*params[i_bestfit2])
                )

                np.save(
                    path
                    + "fit_results/{}/{}/{}/{}_chi2.npy".format(
                        survey, file.replace(".fits", ""), transition, comp
                    ),
                    chi2_grid,
                )
                np.save(
                ...
                np.save(
                    path
                    + "fit_results/{}/{}/{}/{}_parameters.npy".format(
                        survey, file.replace(".fits", ""), transition, comp
                    ),
                    params,
                )
        ...
        try:
            ...
            print(
                "  The best-fitting model has parameters"
...
def model_selection_new(
    path="/mnt/yanitski_backup/yanitski/projects/pdr/KT3_history/MilkyWay",
    missions=None,
    lat=None,
    f_idx=0,
    error_cal=0,
    ...
    for s, survey in enumerate(missions):
        ...
        for f, file in enumerate(obs.files):
            ...
            if (
                not (".fits" in file or ".idl" in file or ".csv" in file)
                or "error" in file
            ):
                continue
            ...
            for i, transition_obs in enumerate(transitions):
                ...
                if PRINT:
                    print(
                        "    Average error in observation: {}".format(
                            np.nanmean(obs_error)
                        )
                    )
                    print(
                        "    Correction due to axisymmetry: {}".format(
                            np.nanmean(obs_error_conf)
                        )
                    )
                ...
                for param in model_params:
                    ...
                    if not os.path.isfile(
                        path + dir_model + "synthetic_intensity.fits"
                    ):
                        print("No intensity")
                        continue
                    ...
                    if (
                        os.path.isdir(
                            path
                            + f"fit_results/{survey}/{file.replace('.fits', '')}/{transition}/"
                        )
                        == False
                    ):
                        os.makedirs(
                    ...
                    i_lat_obs = np.where(
                        (lat_obs >= lat_model.min()) & (lat_obs <= lat_model.max())
                    )[0]

                    ix_err = np.ix_(i_lat_model, i_lon_model)
                    if survey == "COBE-FIRAS":
                        bin_edge = np.array(
                            [*((lon_obs[:-1] + lon_obs[1:]) / 2), 182.5]
                        )
                        if " + " in transition:
                            model_intensity = model.get_species_intensity(
                        ...
                        lon_model_alt[lon_model_alt < -177.5] = (
                            lon_model_alt[lon_model_alt < -177.5] + 360
                        )
                        model_data_temp = binned_statistic(
                            lon_model_alt,
                            model_intensity,
                            statistic="mean",
                            bins=bin_edge,
                        ...
                        model_data = np.array(
                            [model_data_temp[:, -1], *model_data_temp.T]
                        ).T
                        obs_data = np.array(
                            model_data_temp.shape[0]
                            * [obs_data_temp[:, transition_indeces[i]]]
                        )
                        ix = np.ix_(i_lat_model, np.arange(lon_obs.size))
                        ix_obs = np.ix_(*(np.arange(_) for _ in obs_data.shape))
                        obs_error_final = np.array(
                            model_data_temp.shape[0]
                            * [
                                np.sqrt(
                                    obs_error[:, transition_indeces[i]] ** 2
                                    + obs_error_conf[:, transition_indeces[i]] ** 2
                                )
                            ]
                        )[ix_obs][ix]
                        ...
                        bin_mid = obs_lon
                        bin_edge = np.array(
                            [-180, *((bin_mid[:-1] + bin_mid[1:]) / 2), 180]
                        )
                        model_intensity = model.get_species_intensity(
                            transition="C+ 1", include_dust=False, integrated=False
                        )[:, 4, :]
                        model_data = binned_statistic(
                            model.map_lon,
                            model_intensity,
                            statistic="mean",
                            bins=bin_edge,
                        ...
                        for l, lon in enumerate(obs_lon):
                            ...
                            obs_error_final[:, l] = np.sqrt(
                                obs_error[lon_obs == lon].max() ** 2
                                + obs_error_conf[lon_obs == lon] ** 2
                            )
                        ...
                        if comp_type == "integrated":
                            ...
                            obs_error_final = np.sqrt(
                                (np.sqrt(vel_model.size) * obs_error) ** 2
                                + (np.sqrt((obs_error_conf**2).sum(axis=0)) ** 2)
                            )[i_obs][ix_err]
                        elif comp_type == "pv":
                            ix = np.ix_(
                                np.arange(vel_model.size), i_lat_model, i_lon_model
                            )
                            ix_obs = np.ix_(
                                np.arange(vel_model.size), i_lat_obs, i_lon_model
                            )
                        ...
                        if comp_type == "integrated":
                            ix = np.ix_(i_lat_model, i_lon_model)
                            ix_obs = np.ix_(i_lat_obs, i_lon_model)
                            model_data = model.get_species_intensity(
                                transition=transition,
                                include_dust=False,
                                integrated=True,
                            )
                    ...
                    np.save(
                        path
                        + f"fit_results/{survey}/{file.replace('.fits', '')}/{transition}/"
                        f"{dir_model.replace('/', '').replace('channel_intensity.fits', '') + '_' + comp}_chi2.npy",
                        chi2,
                    )
                    np.save(
                        path
                        + f"fit_results/{survey}/{file.replace('.fits', '')}/{transition}/"
                        f"{dir_model.replace('/', '').replace('channel_intensity.fits','') + '_' + comp}_loglikelihood.npy",
                        loglikelihood,
                    )
                ...
                np.save(
                    path
                    + f"fit_results/{survey}/{file.replace('.fits', '')}/{transition}/{comp}_chi2.npy",
                    chi2_grid,
                )
                np.save(
                    path
                    + f"fit_results/{survey}/{file.replace('.fits', '')}/{transition}/{comp}_loglikelihood.npy",
                    loglikelihood_grid,
                )
                np.save(
                    path
                    + f"fit_results/{survey}/{file.replace('.fits', '')}/{transition}/{comp}_parameters.npy",
                    params,
                )
...
def line_ratio_comparison(
    obs_path="/mnt/hpc_backup/yanitski/projects/pdr/observational_data/MilkyWay/",
    ...
    **kwargs,
    ...
    survey_paths = [
        [],
        [],
    ]  # list for paths to plotted data (in order observed, synthetic)
    survey_maps = [
        [],
        [],
    ]  # list for unprocessed (pre-calculation) data to plot (in order observed, synthetic)
    survey_i_lat_init = (
        []
    )  # list for initial observed lattitude indeces where there is an observation
    survey_i_vel_init = (
        []
    )  # list for initial observed velocity indeces where there is an observation
    survey_ix = [
        [],
        [],
    ]  # list of indeces used in comparison (in order observed, synthetic)
    survey_ratios = [
        [],
        [],
    ]  # list of processed (post-calculation) data to plot (in order observed, synthetic)
    ...
    survey_vels = [
        [],
        [],
    ]  # list of data velocities (in order observed, synthetic; 0 for 2D maps)
    ...
    for i, survey in enumerate(missions):
        print(survey)
        survey_paths[0].append(
            obs_path + missions[i] + "/regridded/temp/" + survey_files[i]
        )
        if ".fits" in survey_paths[0][i]:
            obs = fits.open(survey_paths[0][i])
            survey_transitions[0].append(
                np.asarray(obs[0].header["TRANSL"].split(", "))
            )
            i_trans_obs = survey_transitions[0][i] == transitions[i]
            if i_trans_obs.any() == False:
                print(
                    "Invalid transition {} for file {}".format(
                        transitions[i], survey_paths[i]
                    )
                )
                return
            survey_lons[0].append(
                np.linspace(
                    obs[0].header["CRVAL1"]
                    - obs[0].header["CDELT1"] * (obs[0].header["CRPIX1"] - 1),
                    obs[0].header["CRVAL1"]
            ...
            if survey == "COBE-FIRAS":
                ...
                "COBE-FIRAS" in missions or "Planck" in missions
            ) or comp_type == "integrated":
                survey_vels[0].append(
                    np.linspace(
                        obs[0].header["CRVAL3"]
                        - obs[0].header["CDELT3"] * (obs[0].header["CRPIX3"] - 1),
                        obs[0].header["CRVAL3"]
                ...
                print(
                    """Enter a valid string for `comp_type`: 'integrated' for a 2D comparison of maps """
            ...
            if i_trans_obs.any() == False:
                print()
                return
            survey_maps[0].append(
                (
                    obs["amplitude"][:, i_trans_obs]
                    / (2.9979**3)
                    * (cobe_idl_linfrq[i_trans_obs] ** 3)
                    * 2
                    * 1.38
                    / 10**8
                ).reshape(0, -1)
    ...
    if (
        ...
        and comp_type == "pv"
        and not (
            "COBE-FIRAS" in missions or "COBE-DIRBE" in missions or "Planck" in missions
        )
    ):
        lat_min = lat
        lat_max = lat
    ...
    model_params = zip(
        *[model_param_grid[n].flatten() for n in range(len(model_param_grid))]
    )
    ...
    for param in model_params:
        ...
        if len(survey_lats[1]) == 0:
            survey_lats[1].append(
                np.linspace(
                    ...
                    model[1].header["CRVAL3"]
                    + model[1].header["CDELT3"]
                    * (model[1].header["NAXIS3"] - model[1].header["CRPIX3"] - 0.5),
                    num=model[1].header["NAXIS3"],
                )
                * 180
                / np.pi
            ...
            survey_lons[1].append(
                np.linspace(
                    ...
                    model[1].header["CRVAL2"]
                    + model[1].header["CDELT2"]
                    * (model[1].header["NAXIS2"] - model[1].header["CRPIX2"] - 0.5),
                    num=model[1].header["NAXIS2"],
                )
                * 180
                / np.pi
        ...
        for i, transition in enumerate(transitions):
            if transition == "Dust":
                survey_transitions[1].append(
                    np.asarray(model[2].header["DUST"].split(", "))
                )
                ...
                if (
                    i_transition.size == 1
                    and comp_type == "integrated"
                    and not ("COBE-FIRAS" in missions or "Planck" in missions)
                ):
                    survey_maps[1][-1].append(
                        np.trapz(
                            model[1].data[:, :, :, i_transition][:, :, :, 0]
            ...
            if len(survey_ix[0]) < 2:
                i_lon_obs = np.linspace(
                    0, survey_lons[0][i].size - 1, num=survey_lons[0][i].size, dtype=int
                )
                i_lon_model = np.linspace(
                    0, survey_lons[1][0].size - 1, num=survey_lons[1][0].size, dtype=int
                )
                i_lat_model = np.where(
                    (survey_lats[1][0] >= lat_min) & (survey_lats[1][0] <= lat_max)
                )[0]
                i_lat_obs = np.where(
                    (survey_lats[0][i] >= survey_lats[1][0][i_lat_model].min())
                    & (survey_lats[0][i] <= survey_lats[1][0][i_lat_model].max())
                )[0]
                if comp_type == "integrated":  # compare 2D maps
                    ...
                    i_vel_model = np.where(
                        (survey_vels[1][0] >= vel_min) & (survey_vels[1][0] <= vel_max)
                    )[0]
                    i_vel_obs = np.where(
                        (survey_vels[0][i] >= survey_vels[1][0][i_vel_model].min())
                        & (survey_vels[0][i] <= survey_vels[1][0][i_vel_model].max())
                    )[0]
        ...
        if len(survey_ratios[0]) == 0:
            survey_ratios[0].append(
                (
                    survey_maps[0][0][survey_ix[0][0]]
                    / survey_maps[0][1][survey_ix[0][1]]
                ).flatten()
            )
        # for i in range(len(survey_maps[1])):
        survey_ratios[1].append(
            (
                survey_maps[1][-1][0][survey_ix[1][0]]
                / survey_maps[1][-1][1][survey_ix[1][1]]
            ).flatten()
        )
    ...
    if log_comp == True:
        ...
        data = [
            np.log10(survey_ratios[0][0][~i_nan_obs]),
            *[
                np.log10(survey_ratios[1][i][~i_nan_model])
                for i in range(len(survey_ratios[1]))
            ],
        ]
        ...
        if ylabel == "" and comp_type == "integrated":
            ax.set_ylabel(
                r"$log_{10} "
                + r"\left( \varpi_{"
                + r"{}".format(transitions[0])
                + r"} \ / \ \varpi_{"
                + r"{}".format(transitions[1])
                + r"} \right)$",
                fontsize=fontsize,
            )
        if ylabel == "":
    ...
    if save_plot:
        if output_file == "":
            filename = "_".join(file_format.replace("/", "").split("_")[1:]).replace(
                "{}", "_"
            )
            current_output_file = (
                "boxplot_ratio_{}-{}_{}-{}_{}".format(
                    survey_paths[0][0].split("/")[-1].split(".")[0],
                    transitions[0],
        ...
        plt.savefig(
            model_path
            + "fit_results/Plots/{}.{}".format(current_output_file, output_format),
...
def violin_comparison(
    ...
    **kwargs,
    ...
    if surveys == "" or surveys == None or surveys == []:
        ...
        if "COBE" in surveys:
            survey.remove("COBE")
            if "COBE-FIRAS" in surveys:
                surveys.remove("COBE-FIRAS")
            if "COBE-DIRBE" in surveys:
                surveys.remove("COBE-DIRBE")
    ...
    for survey in surveys:
        ...
        if not os.path.exists(
            path.replace("observational_data", "KT3_history") + "fit_results/" + survey
        ):
            os.mkdir(
                path.replace("observational_data", "KT3_history")
                + "fit_results/"
                + survey
            )
        ...
        for survey_file in survey_files:
            ...
            if "error" in survey_file or (
                not ".fits" in survey_file and not ".idl" in survey_file
            ):
                continue
            ...
            if ".fits" in survey_file or ".FITS" in survey_file:
                ...
                obs_error = fits.open(
                    obs_directory + survey_file.replace(".fits", "_error.fits")
                )[0].data
            ...
            for i_trans_obs, transition in enumerate(transitions):
                ...
                for param in deepcopy(model_params):
                    ...
                    if transition == "Dust":
                        ...
                        if i_transition.size and comp_type == "pv":
                            ...
                            print(
                                "Transition {} not found in file {}".format(
                                    transition, survey_file
                                )
                            )
                            break
                    ...
                    i_vel_obs = np.linspace(
                        0, vel_obs.size - 1, num=vel_obs.size, dtype=int
                    )
                    i_vel_model = np.linspace(
                        0, vel_model.size - 1, num=vel_model.size, dtype=int
                    )
                    i_lon_obs = np.linspace(
                        0, lon_obs.size - 1, num=lon_obs.size, dtype=int
                    )
                    i_lon_model = np.linspace(
                        0, lon_model.size - 1, num=lon_model.size, dtype=int
                    )
                    if (
                        survey == "COBE-FIRAS"
                        or survey == "DOBE-DIRBE"
                        or survey == "Planck"
                    ):
                        i_lat_model = (lat_model >= lat_min) & (lat_model <= lat_max)
                        i_lat_obs = (lat_obs >= lat_model[i_lat_model].min()) & (
                    ...
                    if (
                        not survey == "COBE-FIRAS"
                        and not survey == "COBE-DIRBE"
                        and comp_type == "pv"
                    ):
                    ...
                    if comp_type == "pv" and not survey in [
                        "COBE-FIRAS",
                        "COBE-DIRBE",
                        "Planck",
                    ]:
                ...
                if debug:
                    print(
                        "number of maps and labels: {}, {}".format(
                            len(map_list), len(map_labels)
                        )
                    )
                    print("observation")
                    print(
                        "  full map size: {}, indexed map size: {}".format(
                            map_list[0].size, map_list[0][i_obs].size
                        )
                    )
                    if len(map_list) > 1:
                        print("model")
                        print(
                            "  full map size: {}, indexed map size: {}".format(
                                map_list[1].size, map_list[1][i_model].size
                            )
                        )

                fig, ax = plt.subplots(
                    1, 1, figsize=(violin_spacing * len(map_list), 7)
                )
                violin_positions = np.arange(
                    0, violin_spacing * len(map_labels), violin_spacing
                )
                i_nan_obs = (map_list[0][i_obs] <= 0) | np.isnan(map_list[0][i_obs])
                i_nan_model = [
                    ((m[i_model[i]] <= 0) | np.isnan(m[i_model[i]]))
                    for i, m in enumerate(map_list[1:])
                ]
                if log_comp == True:
                    # map_list[0] = np.nan_to_num(map_list[0], nan=0)
                    data = [
                        np.log10(map_list[0][i_obs][~i_nan_obs].flatten()),
                        *[
                    ...
                    if ylabel == "" and comp_type == "integrated":
                        ax.set_ylabel(
                            r"$log_{10} (\varpi) \ \left( K \frac{km}{s} \right)$",
                            fontsize=fontsize,
                        )
                    elif ylabel == "":
                        ax.set_ylabel(
                            r"$log_{10} (T_\mathrm{B}) \ \left( K \right)$",
                            fontsize=fontsize,
                        )
                    else:
                        ax.set_ylabel(ylabel)
                    ...
                    data = [
                        map_list[0][i_obs][~i_nan_obs].flatten(),
                        *[
                    ...
                    if comp_type == "integrated":
                        ax.set_ylabel(
                            r"$\varpi \ \left( K \frac{km}{s} \right)$",
                            fontsize=fontsize,
                        )
                    else:
                        ax.set_ylabel(
                            r"$T_\mathrm{B} \ \left( K \right)$", fontsize=fontsize
                        )
                ax.set_xticks(violin_positions, labels=map_labels)
                ax.xaxis.set_tick_params(
                    labelrotation=label_rotation, labelsize=label_fontsize
                )
                ...
                if save_plot:
                    if output_file == "":
                        current_output_file = (
                            path.replace("observational_data", "KT3_history")
                            + f"fit_results/{survey}/Plots/violinplot_{survey_file}-{transition}"
                            + comp
                        )
                    else:
                        current_output_file = output_file
                    plt.savefig(
                        "{}.{}".format(current_output_file, output_format),
                        format=output_format,
                    )
                    plt.close()
                else:
...
def double_line_plot(
    ...
    **kwargs,
    ...
    for param in model_params:
        ...
        if len(survey_ratios[0]) == 0:
            survey_lines[0].append(
                (
                    survey_maps[0][0][survey_ix[0][0]].flatten(),
                    survey_maps[0][1][survey_ix[0][1]].flatten(),
                )
            )
        # for i in range(len(survey_maps[1])):
        survey_lines[1].append(
            (
                survey_maps[1][-1][0][survey_ix[1][0]].flatten(),
    ...
    if log_comp == True:
        ...
        i_nan_obs = (survey_ratios[0][0] <= 0) | np.isnan(survey_ratios[0][0])
        data = [
            np.log10(survey_lines[0][0][~i_nan_obs]),
    ...
    for i, lines in enumerate(data):
        ...
        if save_plot:
            if output_file == "":
                ...
                current_output_file = (
                    "line_comp_{}-{}_{}-{}==>{}".format(*output_args) + comp
                )
            else:
...
def plot_comparison(
    path="/mnt/hpc_backup/yanitski/projects/pdr/KT3_history/MilkyWay/fit_results/",
    file_format="",
    missions=[],
    model_param=[[]],
    transitions=[],
    ...
    **kwargs,
    ...
    if naxis.size == 2:
        ...
        sub_params = zip(
            *tuple(
                p.flatten()
                for p in np.meshgrid(*np.asarray(model_param, dtype=object)[~naxis])
            )
        )
    ...
    for survey in missions:
        ...
        for param in deepcopy(sub_params):
            ...
            for f, survey_file in enumerate(survey_files):
                ...
                if (
                    ".png" in survey_file
                    or "Plots" in survey_file
                    or ".npy" in survey_file
                    or ".npz" in survey_file
                ):
                    continue
                if not survey_file in os.listdir(path + survey + "/"):
                    print(
                        "File {} not available for survey {}".format(
                            survey_file, survey
                        )
                    )
                    continue
                ...
                if len(transitions_all):
                    ...
                    for t in copy(transitions):
                        ...
                        if debug:
                            print(
                                "  transitions compared: {}\n  transitions skipped: {}".format(
                                    transitions, transitions_skipped
                                )
                            )
                ...
                if len(transitions_skipped) and verbose:
                    print(
                        "  transitions {} not available.".format(
                            ", ".join(transitions_skipped)
                        )
                    )
                ...
                for i in range(log_likelihood.shape[0]):
                    for j in range(log_likelihood.shape[1]):
                        ...
                        if verbose:
                            print(
                                file_format.format(*file_format_param)
                        ...
                        # print([p.sum() for p in param_log_likelihood])
                        for _ in range(len(param_log_likelihood)):
                            param_log_likelihood[_][
                                param_log_likelihood[_] == 0
                            ] = np.nan
                            param_log_likelihood[_][
                                param_log_likelihood[_] >= stat_lim
                            ] = np.nan
                        if likelihood:
                            log_likelihood[i, j] = log_likelihood[i, j] + np.nansum(
                                param_log_likelihood
                            )
                        else:
                            log_likelihood[i, j] = log_likelihood[i, j] + np.nansum(
                        ...
                        for t in range(len(transitions)):
                            transition_log_likelihood[t][i, j] = (
                                transition_log_likelihood[t][i, j]
                                + np.nansum(param_log_likelihood[t])
                            )

                file_transition_plots.append(copy(transition_plots))
                file_transition_log_likelihood.append(
                    deepcopy(transition_log_likelihood)
                )
                np.savez(
                    f"{path}{survey}/{survey_file}/{prefix}{output_file}_{comp}_comparison.npz",
                    trans=file_transitions,
                    chi2=file_transition_log_likelihood,
                )
                ...
                for f in range(len(survey_files)):
                    for t in range(len(file_transitions[f])):
                        if (file_transition_log_likelihood[f][t] < 0).all():
                            file_transition_log_likelihood[f][t] = np.log10(
                                -file_transition_log_likelihood[f][t]
                            )
                        else:
                            file_transition_log_likelihood[f][t] = np.log10(
            ...
            if len(param) == 1:
                sub_indeces = (
                    0,
                    np.asarray(model_param, dtype=object)[~naxis].index(param),
                )
                x_param = param[0]
                y_param = ""
            elif axes.size > 1:
                sub_indeces = tuple(
                    ...
                    for i, arr in enumerate(
                        np.asarray(model_param, dtype=object)[~naxis]
                    )
                )[::-1]
                x_param = param[1]
                y_param = param[0]
            ...
            #     minval,
            #     np.sum(file_dof),
            #     (np.isnan(log_likelihood) | np.isinf(log_likelihood)).any(),
            # )
            # minval = np.around(minval/(10**np.floor(np.log10(minval))), 3) * 10**np.floor(np.log10(minval))
            # print((log_likelihood / np.sum(file_dof) - minval) / minval * 100)

            if contour:
                cm = axes[sub_indeces].contourf(
                    (log_likelihood / np.sum(file_dof) - minval) / minval * 100,
                    antialiased=True,
                    levels=levels,
                    cmap=cmap,
                )
            else:
                cm = axes[sub_indeces].imshow(
                    log_likelihood / np.sum(file_dof),
                    extent=[0, lenaxis[i_x], 0, lenaxis[i_y]],
                    cmap=cmap,
                )
            axes[sub_indeces].set_aspect(lenaxis[i_x] / ax_aspect / lenaxis[i_y])
            cb = fig.colorbar(
                cm, ax=axes[sub_indeces], fraction=fraction, aspect=cb_aspect
            )
            ...
            axes[sub_indeces].text(
                *offset_idx,
                f"{minval:1.3e} +",
                transform=axes[sub_indeces].transAxes,
                fontsize=labelsize,
                ha="left",
                va="bottom",
            )
            ...
            if axes.size > 1:
                axes[sub_indeces].set_title(
                    "{} {}, {} {}".format(supylabel, y_param, supxlabel, x_param),
                    fontsize=fontsize - 4,
                )
            ...
            for f in range(len(survey_files)):
                for t in range(len(file_transitions[f])):
                    ...
                    if contour:
                        cm = file_transition_plots[f][t][1][sub_indeces].contourf(
                            (file_transition_log_likelihood[f][t] - minval)
                            / minval
                            * 100,
                            antialiased=True,
                            levels=levels,
                        ...
                        cm = file_transition_plots[f][t][1][sub_indeces].imshow(
                            file_transition_log_likelihood[f][t],
                            extent=[0, lenaxis[i_x], 0, lenaxis[i_y]],
                            cmap=cmap,
                        )
                    file_transition_plots[f][t][1][sub_indeces].set_aspect(
                        lenaxis[i_x] / ax_aspect / lenaxis[i_y]
                    )
                    cb = file_transition_plots[f][t][0].colorbar(
                        cm,
                        ax=file_transition_plots[f][t][1][sub_indeces],
                        fraction=fraction,
                        aspect=cb_aspect,
                    )
                    # cb.ax.ticklabel_format(useOffset=minval, style='plain', useMathText=True)
                    ...
                    #
                    # cb.ax.ticklabel_format(
                    #     useOffset=False, style="plain", useMathText=True
                    # )
                    #
                    #
                    # cb.ax.yaxis.offsetText.set_text('')
                    ...
                    file_transition_plots[f][t][1][sub_indeces].text(
                        *offset_idx,
                        f"{minval:1.3e} +",
                        transform=file_transition_plots[f][t][1][sub_indeces].transAxes,
                        fontsize=labelsize,
                        ha="left",
                        va="bottom",
                    )
                    ...
                    file_transition_plots[f][t][1][sub_indeces].set_xticklabels(
                        xtick_labels, rotation=xrot
                    )
                    file_transition_plots[f][t][1][sub_indeces].set_yticks(ytick_base)
                    file_transition_plots[f][t][1][sub_indeces].set_yticklabels(
                        ytick_labels, rotation=yrot
                    )
                    # if contour:
                    ...
                    file_transition_plots[f][t][1][sub_indeces].set_xlabel(
                        xlabel, fontsize=fontsize
                    )
                    file_transition_plots[f][t][1][sub_indeces].set_ylabel(
                        ylabel, fontsize=fontsize
                    )
                    file_transition_plots[f][t][1][sub_indeces].tick_params(
                        labelsize=labelsize
                    )
                    file_transition_plots[f][t][1][sub_indeces].tick_params(
                        axis="x", pad=10
                    ...
                    if file_transition_plots[f][t][1].size > 1:
                        file_transition_plots[f][t][1][sub_indeces].set_title(
                            "{} {}, {} {}".format(
                                supylabel, y_param, supxlabel, x_param
                            ),
                            fontsize=fontsize - 4,
                        )
        ...
        if save_plot:
            ...
            if likelihood:
                plt.savefig(
                    path
                    + survey
                    + "/Plots/{}_{}_loglikelihood.{}".format(
                        output_file, comp, output_format
                    ),
                    format=output_format,
                ...
                plt.savefig(
                    path
                    + survey
                    + "/Plots/{}_{}_chi2.{}".format(output_file, comp, output_format),
                    format=output_format,
                    transparent=transparent,
                )
        else:
        ...
        for f, file in enumerate(survey_files):
            for t in range(len(file_transitions[f])):
                ...
                file_transition_plots[f][t][0].text(
                    clabel_xa,
                    0.5,
                    clabel,
                    rotation=90,
                ...
                file_transition_plots[f][t][0].subplots_adjust(
                    left=pad * pad_left,
                    right=fig_right,
                    bottom=pad * pad_bottom,
                    top=fig_top,
                ...
                if save_plot:
                    ...
                    if likelihood:
                        plt.savefig(
                            path
                            + survey
                            + "/Plots/{}_{}-{}_{}_loglikelihood.{}".format(
                                output_file,
                                file,
                                file_transitions[f][t].replace(" ", "-"),
                                comp,
                                output_format,
                            ),
                            format=output_format,
    ...
    for param in deepcopy(sub_params):
        ...
        if len(param) == 1:
            ...
            sub_indeces = tuple(
                arr.index(param[i])
                for i, arr in enumerate(np.asarray(model_param, dtype=object)[~naxis])
            )[::-1]
            x_param = param[1]
            y_param = param[0]
        else:
        ...
        # minimum chi^2
        minval = (
            loglikelihood_overall[:, :, sub_indeces[0], sub_indeces[1]] / overall_dof
        ).min()
        # minval = np.around(minval/(10**np.floor(np.log10(minval))), 3) * 10**np.floor(np.log10(minval))

        if contour:
            cm = axes_overall[sub_indeces].contourf(
                (
                    loglikelihood_overall[:, :, sub_indeces[0], sub_indeces[1]]
                    / overall_dof
                    - minval
            ...
            cm = axes_overall[sub_indeces].imshow(
                loglikelihood_overall[:, :, sub_indeces[0], sub_indeces[1]]
                / overall_dof,
                extent=[0, lenaxis[i_x], 0, lenaxis[i_y]],
                cmap=cmap,
        ...
        if axes_overall.size > 1:
            axes_overall[sub_indeces].set_title(
                "{} {}, {} {}".format(supylabel, y_param, supxlabel, x_param),
                fontsize=fontsize - 4,
            )
    ...
    if save_plot:
        ...
        if likelihood:
            ...
            plt.savefig(
                path + "/Plots/{}_{}_chi2.{}".format(output_file, comp, output_format),
                format=output_format,
                transparent=transparent,
            )
    else:
