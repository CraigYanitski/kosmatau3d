...
def interclump_dust_tb(file="interclumpDustTmb.dat"):
    ...
    observations.interclump_dust_tb_centerline = (
        tb.loc[:, ["n", "M", "chi"]].to_numpy(),
        tb.loc[:, "3.1mm":].to_numpy(),
    )
    return
...
def interclump_dust_tau(file="interclumpDustTau.dat"):
    ...
    observations.interclump_dust_tau_centerline = (
        tau.loc[:, ["n", "M", "chi"]].to_numpy(),
        tau.loc[:, "3.1mm":].to_numpy(),
    )
    return
...
def clump_taufuv(file="RhoMassAFUV.dat"):
    ...
    observations.clump_taufuv = (
        taufuv.loc[:, ["n", "M"]].to_numpy(),
        taufuv.loc[:, "tauFUV"].to_numpy(),
    )
    return
...
def interclump_taufuv(file="interclumpTauFUV.dat"):
    ...
    """
    taufuv = pd.read_csv(constants.GRIDPATH + file, sep="\s+")
    observations.interclump_taufuv = (
        taufuv.loc[:, ["n", "M", "chi"]].to_numpy(),
        taufuv.loc[:, "tauFUV"].to_numpy(),
    )
    return
...
def clump_column_density(file="clumpMeanCols.dat"):
    ...
    observations.clump_column_density = (
        N.loc[:, ["n", "M", "chi"]],
        N.loc[:, "ELECTR":],
    )
    return
...
def interclump_column_density(file="interclumpMeanCols.dat"):
    """Open file containing the column densities."""
    N = pd.read_csv(constants.GRIDPATH + file, sep="\s+")
    observations.interclump_column_density = (
        N.loc[:, ["n", "M", "chi"]],
        N.loc[:, "ELECTR":],
    )
    return
...
def clump_temperature(file="clumpTemperatures.dat"):
    """Open file containing the column densities."""
    N = pd.read_csv(constants.GRIDPATH + file, sep="\s+")
    observations.clump_temperature = (
        N.loc[:, ["n", "M", "chi"]],
        N.loc[:, ("Tg", "Td")],
    )
    return
...
def interclump_temperature(file="interclumpTemperatures.dat"):
    """Open file containing the column densities."""
    N = pd.read_csv(constants.GRIDPATH + file, sep="\s+")
    observations.interclump_temperature = (
        N.loc[:, ["n", "M", "chi"]],
        N.loc[:, ("Tg", "Td")],
    )
    return
...
def species_data(file="frequencies.dat"):
    ...
    frequencies = np.genfromtxt(
        constants.GRIDPATH + file,
        names=["number", "species", "transition", "frequency"],
        dtype="i8,U8,i8,f8",
        delimiter=",",
    )
    observations.species_data = (
        frequencies["number"],
        frequencies["species"],
...
def h2_surface_mass_profile(file="h2_surface-density.dat"):
    ...
    if constants.directory != "":
        sigma_h2 = pd.read_csv(
            constants.INPUTPATH + constants.directory + file, sep="\s+", skiprows=1
        )
        ...
        #                        for _ in range(n_vox+1)))
        observations.h2_surface_mass_profile = (
            sigma_h2["r"] * 1000,
            sigma_h2["Sigma_h2"],
        )
...
def hi_surface_mass_profile(file="hi_surface-density.dat"):
    ...
    if constants.directory != "":
        ...
        #                        for _ in range(n_vox+1)))
        observations.hi_surface_mass_profile = (
            sigma_hi["r"] * 1000,
            sigma_hi["Sigma_hi"],
        )
...
def number_density_profile(file="number-densities.dat"):
    ...
    if constants.directory != "":
        density = np.genfromtxt(
            constants.INPUTPATH + constants.directory + file,
            names=["radius", "h2_density"],
        )
        observations.number_density_profile = (
            (density["radius"] * 1000),
            density["h2_density"],
        )
    return
...
def fuv_profile(file="galactic_FUV_complete.dat"):
    ...
    if constants.directory != "":
        ...
        # fuv = 50/4/np.pi/r**2
        observations.fuv_profile = (
            fuv[:, :2],
            fuv[:, 2] / (constants.pc * 100) ** 3 / constants.u_draine0,
...
def galaxy_rotation_profile(file="rot_milki2018_14.dat"):
    ...
    if constants.directory != "":
        ...
        observations.galaxy_rotation_profile = (
            rotation[:, 0] * 1000.0,
            rotation[:, 1:],
        )
    return
