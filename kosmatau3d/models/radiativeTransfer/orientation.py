...
def calculateObservation(directory='', dim='xy', slRange=[(-np.pi,np.pi), (-np.pi/2,np.pi/2)], nsl=[50,25],
    ...
    radiativeTransfer.tempSpeciesEmissivity = fits.open(directory+'/species_emissivity.fits', mode='denywrite')
    radiativeTransfer.tempSpeciesAbsorption = fits.open(directory+'/species_absorption.fits', mode='denywrite')
    radiativeTransfer.tempDustEmissivity = fits.open(directory+'/dust_emissivity.fits', mode='denywrite')
    radiativeTransfer.tempDustAbsorption = fits.open(directory+'/dust_absorption.fits', mode='denywrite')
    ...
    if nDust > 1:
        ...
        radiativeTransfer.interpDust = interpolate.interp1d(constants.wavelengths[:nDust],
                                                            radiativeTransfer.tempDustEmissivity[0].data[:, 0, :],
                                                            fill_value='extrapolate')
    
    constants.velocityRange = np.linspace(radiativeTransfer.tempSpeciesEmissivity[0].header['CRVAL2'] -
                                          (radiativeTransfer.tempSpeciesEmissivity[0].header['CRPIX2']) *
                                          radiativeTransfer.tempSpeciesEmissivity[0].header['CDELT2'],
                                          radiativeTransfer.tempSpeciesEmissivity[0].header['CRVAL2'] +
                                          (radiativeTransfer.tempSpeciesEmissivity[0].header['CRPIX2']) *
                                          radiativeTransfer.tempSpeciesEmissivity[0].header['CDELT2'],
    ...
    xPositions, yPositions, zPositions = radiativeTransfer.voxelPositions[0].data[:, 0], \
        radiativeTransfer.voxelPositions[0].data[:, 1], \
        radiativeTransfer.voxelPositions[0].data[:, 2]
    ...
    if constants.fromEarth:
        ...
        # radiativeTransfer.vTqdm = tqdm(total=constants.velocityRange.size, desc='Observing velocity',
        #                                miniters=1, dynamic_ncols=True)
        # if terminal: radiativeTransfer.slTqdm = tqdm(total=longrid.size*latgrid.size, desc='Sightline',
        #                                miniters=1, dynamic_ncols=True)
...
def calculateVelocityChannel(ivelocity, slRange=[(-np.pi,np.pi), (-np.pi/2,np.pi/2)], nsl=[50,25],
    ...
    if nDust > 1:
        ...
        base = radiativeTransfer.tempDustEmissivity[0].data[0, i_vel, 0]
      
    radiativeTransfer.i_vox = ((radiativeTransfer.tempSpeciesEmissivity[0].data[:, i_vel, :] > base).any(1) |
                               radiativeTransfer.tempDustEmissivity[0].data[:, i_vel, :].any(1))
    ...
    if multiprocess == 0:
        radiativeTransfer.slTqdm.reset()
    ...
    if radiativeTransfer.i_vox.any() is False:
        # print('\n\n', [], '\n\n')
    ...
    for j, lat in enumerate(latgrid):
        ...
        for i, lon in enumerate(longrid):
            ...
            if multiprocess == 0:
                radiativeTransfer.slTqdm.update()
...
def setLOS(x=0, y=0, z=0, lon=0, lat=0, i_vox=[], i_vel=0, i_spe=None, i_dust=None,
    ...
    xGrid, yGrid, zGrid = radiativeTransfer.voxelPositions[0].data[:,0], \
                          radiativeTransfer.voxelPositions[0].data[:,1], \
                          radiativeTransfer.voxelPositions[0].data[:,2]  # grid.getVoxelPositions()
    ...
    if iLoS.size == 1:
        ...
        radiativeTransfer.intensitySpecies = scale * radiativeTransfer.tempSpeciesEmissivity[0].data[iLoS, i_vel, :][0, :] / \
                                             constants.pc/100
        radiativeTransfer.intensityDust = scale * radiativeTransfer.tempDustEmissivity[0].data[iLoS, i_vel, :][0, :] / \
                                          constants.pc/100
        ...
        #       (radiativeTransfer.tempSpeciesEmissivity[0].data[iLoS_ordered,i_vel,:]>0).any(), '\n\n')
    ...
    return 2, vel.size
    # ,(epsilonSpecies,epsilonStepSpecies,kappaSpecies,kappaStepSpecies,
    #   epsilonDust,epsilonStepDust,kappaDust,kappaStepDust)
...
def calculateRadiativeTransfer(scale=1, backgroundI=0., species=True, dust=True, verbose=False, test=True):
    ...
    with warnings.catch_warnings():
        ...
        if species:
            aSpecies = (radiativeTransfer.kappaSpecies[:-1, :] /
                        np.sqrt(2*radiativeTransfer.kappaStepSpecies.astype(np.complex_)))
            bSpecies = ((radiativeTransfer.kappaSpecies[:-1, :] + radiativeTransfer.kappaStepSpecies*scale) /
                        np.sqrt(2*radiativeTransfer.kappaStepSpecies.astype(np.complex_)))
        if dust:
            aDust = (radiativeTransfer.kappaDust[:-1, :] /
                     np.sqrt(2*radiativeTransfer.kappaStepDust.astype(np.complex_)))
            bDust = ((radiativeTransfer.kappaDust[:-1, :] + radiativeTransfer.kappaStepDust*scale) /
                     np.sqrt(2*radiativeTransfer.kappaStepDust.astype(np.complex_)))
    
    if verbose:
        print(radiativeTransfer.kappaSpecies.shape[0])
    
    for i in range(nSteps):
    
        # Determine which form of integration the species transitions require
        if species:
            k0Species = ((radiativeTransfer.kappaStepSpecies[i, :] == 0) &
                         (abs(radiativeTransfer.kappaSpecies[:-1, :][i, :]*scale) < 10**-10))
            try:
                kgSpecies = ~k0Species & (radiativeTransfer.kappaSpecies[:-1, :][i, :] >
                                          10**3*abs(radiativeTransfer.kappaStepSpecies[i, :])*scale)
                ...
                input()
                kgSpecies = ~k0 & (radiativeTransfer.kappaSpecies[:-1, :][i, :] >
                                   10**3*abs(radiativeTransfer.kappaStepSpecies[i, :])*scale)
        ...
        if dust:
            k0Dust = ((radiativeTransfer.kappaStepDust[i, :] == 0) &
                      (abs(radiativeTransfer.kappaDust[:-1, :][i, :]*scale) < 10**-10))
            try:
                kgDust = ~k0Dust & (radiativeTransfer.kappaDust[:-1, :][i, :] >
                                    10**3*abs(radiativeTransfer.kappaStepDust[i, :])*scale)
            except RuntimeWarning:
                # print(radiativeTransfer.x1LoS, radiativeTransfer.x2LoS)
                # print(radiativeTransfer.x3LoS)
                ...
                input()
                kgDust = ~k0 & (radiativeTransfer.kappaDust[:-1, :][i, :] >
                                10**3*abs(radiativeTransfer.kappaStepDust[i, :])*scale)
        ...
        if species:
            if k0Species.any():
                radiativeTransfer.intensitySpecies[k0Species] = (radiativeTransfer.intensitySpecies[k0Species] +
                                                                 radiativeTransfer.epsilonSpecies[:-1, :][i, :][k0Species] *
                                                                 scale +
                                                                 0.5*radiativeTransfer.epsilonStepSpecies[i, :][k0Species]) * \
                                                                scale**2
        
            elif kgSpecies.any():
                try:
                    radiativeTransfer.intensitySpecies[kgSpecies] = np.exp(-radiativeTransfer.kappaSpecies[i, :][kgSpecies]*scale) * radiativeTransfer.intensitySpecies[kgSpecies] + ((radiativeTransfer.epsilonSpecies[i, :][kgSpecies]*radiativeTransfer.kappaSpecies[i, :][kgSpecies]+radiativeTransfer.epsilonStepSpecies[i, :][kgSpecies]*(radiativeTransfer.kappaSpecies[i, :][kgSpecies]*scale-1))/(radiativeTransfer.kappaSpecies[i, :][kgSpecies]**2.)) - np.exp(-radiativeTransfer.kappaSpecies[i, :][kgSpecies]*scale)*((radiativeTransfer.epsilonSpecies[i, :][kgSpecies]*radiativeTransfer.kappaSpecies[i, :][kgSpecies]-radiativeTransfer.epsilonStepSpecies[i, :][kgSpecies])/(radiativeTransfer.kappaSpecies[i, :][kgSpecies]**2.))
                except RuntimeWarning:
                    print('ERROR: Integration option 2')
                    radiativeTransfer.intensitySpecies[kgSpecies] = np.exp(-radiativeTransfer.kappaSpecies[i, :][kgSpecies]*scale) * (radiativeTransfer.intensitySpecies[kgSpecies] + ((radiativeTransfer.epsilonSpecies[i, :][kgSpecies]*radiativeTransfer.kappaSpecies[i, :][kgSpecies]+radiativeTransfer.epsilonStepSpecies[i, :][kgSpecies]*(radiativeTransfer.kappaSpecies[i, :][kgSpecies]*scale-1))/(radiativeTransfer.kappaSpecies[i, :][kgSpecies]**2.)) - ((radiativeTransfer.epsilonSpecies[i, :][kgSpecies]*radiativeTransfer.kappaSpecies[i, :][kgSpecies]-radiativeTransfer.epsilonStepSpecies[i, :][kgSpecies])/(radiativeTransfer.kappaSpecies[i, :][kgSpecies]**2.)))
                ...
                radiativeTransfer.intensitySpecies[kEgSpecies] = (radiativeTransfer.epsilonStepSpecies[i, :][kEgSpecies]/radiativeTransfer.kappaStepSpecies[i, :][kEgSpecies]*(1-np.exp(-radiativeTransfer.kappaSpecies[:-1, :][i, :][kEgSpecies]*scale-radiativeTransfer.kappaStepSpecies[i, :][kEgSpecies]/2.*scale**2.)) - (radiativeTransfer.epsilonSpecies[:-1, :][i, :][kEgSpecies]*radiativeTransfer.kappaStepSpecies[i, :][kEgSpecies]-radiativeTransfer.epsilonStepSpecies[i, :][kEgSpecies]*radiativeTransfer.kappaSpecies[:-1, :][i, :][kEgSpecies])/radiativeTransfer.kappaStepSpecies[i, :][kEgSpecies] * np.sqrt(np.pi/(2.*abs(radiativeTransfer.kappaStepSpecies[i][kEgSpecies].astype(np.complex_)))) * (np.exp(aSpecies[i, :][kEgSpecies]**2.-bSpecies[i, :][kEgSpecies]**2.)*aE-bE) + radiativeTransfer.intensitySpecies[kEgSpecies]*np.exp(-radiativeTransfer.kappaSpecies[:-1][i, :][kEgSpecies]*scale-radiativeTransfer.kappaStepSpecies[i, :][kEgSpecies]/2.*scale**2.)).real
                ...
                try:
                    radiativeTransfer.intensitySpecies[kElSpecies] = (radiativeTransfer.epsilonStepSpecies[i, :][kElSpecies]/radiativeTransfer.kappaStepSpecies[i, :][kElSpecies]*(1-np.exp(-radiativeTransfer.kappaSpecies[:-1, :][i, :][kElSpecies]*scale-radiativeTransfer.kappaStepSpecies[i, :][kElSpecies]/2.*scale**2.))-(radiativeTransfer.epsilonSpecies[:-1, :][i, :][kElSpecies]*radiativeTransfer.kappaStepSpecies[i, :][kElSpecies]-radiativeTransfer.epsilonStepSpecies[i, :][kElSpecies]*radiativeTransfer.kappaSpecies[:-1, :][i, :][kElSpecies])/radiativeTransfer.kappaStepSpecies[i, :][kElSpecies] * np.sqrt(np.pi/(2.*abs(radiativeTransfer.kappaStepSpecies[i][kElSpecies].astype(np.complex_)))) * (np.exp(aSpecies[i, :][kElSpecies]**2.-bSpecies[i, :][kElSpecies]**2.)*aE-bE) + radiativeTransfer.intensitySpecies[kElSpecies]*np.exp(-radiativeTransfer.kappaSpecies[:-1, :][i, :][kElSpecies]*scale-radiativeTransfer.kappaStepSpecies[i, :][kElSpecies]/2.*scale**2.)).real
                except RuntimeWarning:
        ...
        if dust:
            if k0Dust.any():
                ...
                try:
                    radiativeTransfer.intensityDust[kgDust] = np.exp(-radiativeTransfer.kappaDust[i, :][kgDust]*scale) * radiativeTransfer.intensityDust[kgDust] + ((radiativeTransfer.epsilonDust[i, :][kgDust]*radiativeTransfer.kappaDust[i, :][kgDust]+radiativeTransfer.epsilonStepDust[i, :][kgDust]*(radiativeTransfer.kappaDust[i, :][kgDust]*scale-1))/(radiativeTransfer.kappaDust[i, :][kgDust]**2.)) - np.exp(-radiativeTransfer.kappaDust[i, :][kgDust]*scale)*((radiativeTransfer.epsilonDust[i, :][kgDust]*radiativeTransfer.kappaDust[i, :][kgDust]-radiativeTransfer.epsilonStepDust[i, :][kgDust])/(radiativeTransfer.kappaDust[i, :][kgDust]**2.))
                except RuntimeWarning:
                    print('ERROR: Integration option 2')
                    radiativeTransfer.intensityDust[kgDust] = np.exp(-radiativeTransfer.kappaDust[i, :][kgDust]*scale) * (radiativeTransfer.intensityDust[kgDust] + ((radiativeTransfer.epsilonDust[i, :][kgDust]*radiativeTransfer.kappaDust[i, :][kgDust]+radiativeTransfer.epsilonStepDust[i, :][kgDust]*(radiativeTransfer.kappaDust[i, :][kgDust]*scale-1))/(radiativeTransfer.kappaDust[i, :][kgDust]**2.)) - ((radiativeTransfer.epsilonDust[i, :][kgDust]*radiativeTransfer.kappaDust[i, :][kgDust]-radiativeTransfer.epsilonStepDust[i, :][kgDust])/(radiativeTransfer.kappaDust[i, :][kgDust]**2.)))
                ...
                radiativeTransfer.intensityDust[kEgDust] = (radiativeTransfer.epsilonStepDust[i, :][kEgDust]/radiativeTransfer.kappaStepDust[i, :][kEgDust]*(1-np.exp(-radiativeTransfer.kappaDust[:-1, :][i, :][kEgDust]*scale-radiativeTransfer.kappaStepDust[i, :][kEgDust]/2.*scale**2.)) - (radiativeTransfer.epsilonDust[:-1, :][i, :][kEgDust]*radiativeTransfer.kappaStepDust[i, :][kEgDust]-radiativeTransfer.epsilonStepDust[i, :][kEgDust]*radiativeTransfer.kappaDust[:-1, :][i, :][kEgDust])/radiativeTransfer.kappaStepDust[i, :][kEgDust] * np.sqrt(np.pi/(2.*abs(radiativeTransfer.kappaStepDust[i][kEgDust].astype(np.complex_)))) * (np.exp(aDust[i, :][kEgDust]**2.-bDust[i, :][kEgDust]**2.)*aE-bE) + radiativeTransfer.intensityDust[kEgDust]*np.exp(-radiativeTransfer.kappaDust[:-1][i, :][kEgDust]*scale-radiativeTransfer.kappaStepDust[i, :][kEgDust]/2.*scale**2.)).real
                ...
                try:
                    radiativeTransfer.intensityDust[kElDust] = (radiativeTransfer.epsilonStepDust[i, :][kElDust]/radiativeTransfer.kappaStepDust[i, :][kElDust] * (1-np.exp(-radiativeTransfer.kappaDust[:-1, :][i, :][kElDust]*scale - radiativeTransfer.kappaStepDust[i, :][kElDust]/2.*scale**2.)) - (radiativeTransfer.epsilonDust[:-1, :][i, :][kElDust] * radiativeTransfer.kappaStepDust[i, :][kElDust] - radiativeTransfer.epsilonStepDust[i, :][kElDust]*radiativeTransfer.kappaDust[:-1, :][i, :][kElDust])/radiativeTransfer.kappaStepDust[i, :][kElDust] * np.sqrt(np.pi/(2.*abs(radiativeTransfer.kappaStepDust[i][kElDust].astype(np.complex_)))) * (np.exp(aDust[i, :][kElDust]**2.-bDust[i, :][kElDust]**2.)*aE-bE) + radiativeTransfer.intensityDust[kElDust]*np.exp(-radiativeTransfer.kappaDust[:-1, :][i, :][kElDust]*scale-radiativeTransfer.kappaStepDust[i, :][kElDust]/2.*scale**2.)).real
                except RuntimeWarning:
    ...
    for i in range(a.size):
        ...
        if kstep[i] == 0 and (k[i]*scale) < 10**-10:
            ...
            df.loc[[i], ['dI']] = (intensity*np.exp(-k[i]*scale-0.5*kstep[i]*scale**2) +
                                   ...
                                   (np.exp(a[i]**2-b[i]**2)*radiativeTransfer.Ereal(np.array([a[i]]), verbose=True) -
                                    radiativeTransfer.Ereal(np.array([b[i]]), verbose=True)).real)
            ...
            df.loc[[i], ['dI']] = (intensity*np.exp(-k[i]*scale-0.5*kstep[i]*scale**2) +
                                   estep[i]/kstep[i]*(1-np.exp(-k[i]*scale-0.5*kstep[i]*scale**2)) -
                                   (e[i]*kstep[i]-estep[i]*k[i])/kstep[i]*np.sqrt(np.pi/2/abs(kstep[i])) *
                                   (np.exp(a[i]**2-b[i]**2)*radiativeTransfer.Eimag(np.array([a[i]]), verbose=True) -
                                    radiativeTransfer.Eimag(np.array([b[i]]), verbose=True)).real)
